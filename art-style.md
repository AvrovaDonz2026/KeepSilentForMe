...
不要改成甜美 VTuber 立绘。沿用母版人物逻辑；当前 V4 包含 10 个基础表情和 2 个变体，映射以 `art/v4/playable/manifest.json` 的 `faceMap` 为准：

| 项 | 做法 |
| --- | --- |
| 造型 | 长黑发、连帽衫或朴素居家/出门层；可加一件“出门伪装”（薄外套、耳麦）表示直播/面试，但颜色仍低饱和 |
| 表演脸 | 10 个基础表情 + 2 个变体：**假笑、紧张、冷淡、空白、依赖、崩溃边缘、讨好、抽离**等——全是微表情，不靠夸张 Q 版 |
...
| 资产 | 规格建议 | 备注 |
| --- | --- | --- |
| 少女关键姿势 | V4 运行时 8 张（背、侧、半身对镜/对屏、低头） | 优先侧/背降低表情成本；ID 见 manifest |
| 表情差分 | 10 个基础表情 + 2 个变体 | 眼神与嘴角微变即可；运行时 ID 见 V4 manifest |
| 消音体 | 3 阶段 × 可 tile 的墨迹/字屑 | 可程序叠字 |
| 场景 BG | 5 张旧批次 PNG（1536×1024）；运行时逻辑画布 1920×1080 | 面试/直播桌/门厅/同桌加压/空房，按 manifest 做 fit/crop |
| 道具 | CRT/显示器、台灯、旧椅、雨窗 | 直接复用母版物件语言 |
| UI / 交互层 | V4 对话框、黑条、直播点、门/弹幕/裂纹/碎字 FX | 与原画分层；高对比但不高饱和 |
| 海报/标题 | 1 张 | 她侧脸 + 嘴边爬字人形；近黑底 |

母版/关键帧仍可用 `2048×1152`（16:9）；V4 运行时透明层必须以
`art/v4/playable/manifest.json` 的逐资产尺寸为准，旧 BG 由运行时 fit/crop 到
`1920×1080` 逻辑画布。
...
## 十、关末 AI 视频（与静帧同一 DNA）

关间转场、章结局、终局与反转使用 **图生视频**（见 `台本.md` / `schedule.md` §4.5）。

| 项 | 规范 |
| --- | --- |
| 画风 | 必须延续本节 doomer 2D 线稿；禁止切到实拍或甜系 3D |
| 首帧 | 优先 Demo 锚点（D0–D5）或 R0 道具锁 |
| 运动 | 慢推/慢横移/焦点肩侧消音体；避免手持晃 |
| 消音体 | 视频中仍为黑条+碎字堆叠，无五官、无霓虹 |
| 声音 | 环境声为主，默认无 BGM（与静帧「安静」一致） |
| 文字 | 不把长旁白烧进画面；UI 叠字 |

---

*v0.2 · 美术风格锁定：Doomer-1999 关键帧 + 关末图生视频 → 《请替我沉默》全案默认视觉。*
